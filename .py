...
"Sophian investi dans des paris sportifs grâce à ChatGPT, mais reste en négatif.",
    ...
    def show_whack_a_psyduck_game(self):
        """Afficher le mini-jeu Tape-Taupe avec Psyduck"""
        # Créer une fenêtre popup pour le jeu
        game_popup = tk.Toplevel(self.root)
        game_popup.title("🎮 Assomme le Psyduck !")
        game_popup.geometry("650x600")
        ...
        subtitle_label = tk.Label(
            title_frame,
            text="Clique sur Psyduck 3 fois pour obtenir un secret !",
            font=("Arial", 11),
            bg="#1a1a2e",
            fg="#a0a0a0"
        )
        ...
        try:
            ...
            game_state['psyduck_image'] = ImageTk.PhotoImage(psyduck_game_img)
        except Exception as e:
            print(f"Erreur chargement psyduck pour le jeu: {e}")
            game_state['psyduck_image'] = None
        ...
        for row in range(3):
            for col in range(3):
                ...
                holes.append(hole_frame)
                hole_labels.append(hole_label)
                
                # Binding pour le clic sur le trou
                def make_click_handler(idx):
                    return lambda e: on_hole_click(idx)
                
                hole_label.bind("<Button-1>", make_click_handler(hole_index))
        ...
        def on_hole_click(hole_idx):
            """Gestionnaire de clic sur un trou"""
            if not game_state['game_active']:
                return
            
            if game_state['psyduck_visible'] and game_state['current_hole'] == hole_idx:
                # TOUCHÉ !
                game_state['hits'] += 1
                score_label.config(text=f"🎯 Score: {game_state['hits']} / 3")
                
                # Effet visuel de touche
                hole_labels[hole_idx].config(bg="#00ff00")
                game_popup.after(150, lambda: hole_labels[hole_idx].config(bg="#0d1117"))
                
                # Cacher le psyduck immédiatement
                hide_psyduck()
                
                # Vérifier la victoire
                if game_state['hits'] >= game_state['target_hits']:
                    game_state['game_active'] = False
                    show_victory()
                else:
                    # Continuer le jeu après un court délai
                    game_popup.after(500, show_psyduck_random)
            else:
                # RATÉ - effet visuel
                if game_state['current_hole'] is not None:
                    hole_labels[game_state['current_hole']].config(bg="#ff4444")
                    game_popup.after(150, lambda h=game_state['current_hole']: hole_labels[h].config(bg="#0d1117") if game_state['current_hole'] != h or not game_state['psyduck_visible'] else None)
        
        def show_psyduck_random():
            ...
            game_state['current_hole'] = new_hole
            game_state['psyduck_visible'] = True
            
            # Afficher le psyduck
            if game_state['psyduck_image']:
                hole_labels[new_hole].config(image=game_state['psyduck_image'], bg="#2d3748")
            else:
                hole_labels[new_hole].config(text="🦆", font=("Arial", 40), bg="#2d3748")
            
            # Programmer la disparition après un délai (800-1200ms)
            hide_delay = random.randint(800, 1200)
            game_state['game_timer'] = game_popup.after(hide_delay, auto_hide_and_next)
        
        def auto_hide_and_next():
            """Cacher le psyduck et en afficher un autre"""
            if game_state['psyduck_visible'] and game_state['game_active']:
                hide_psyduck()
                # Afficher un nouveau psyduck après un court délai
                game_popup.after(300, show_psyduck_random)
        
        def hide_psyduck():
            """Cacher le psyduck actuel"""
            if game_state['current_hole'] is not None:
                hole_labels[game_state['current_hole']].config(image='', text='', bg="#0d1117")
            game_state['psyduck_visible'] = False
            if game_state['game_timer']:
                game_popup.after_cancel(game_state['game_timer'])
        ...
        def show_victory():
            ...
            result_label.config(
                text=f"🎉 BRAVO ! Tu as assommé Psyduck !\n\n💬 \"{message}\"",
                fg="#ffd700",
                font=("Arial", 12, "bold")
            )
        ...
        def restart_game():
            """Redémarrer le jeu"""
            game_state['hits'] = 0
            game_state['current_hole'] = None
            game_state['game_active'] = True
            game_state['psyduck_visible'] = False
            
            score_label.config(text="🎯 Score: 0 / 3")
            result_label.config(text="", font=("Arial", 11))
        ...
        def replay_on_enter(e):
            replay_btn.config(bg="#047857")
        def replay_on_leave(e):
            replay_btn.config(bg="#059669")
        replay_btn.bind("<Enter>", replay_on_enter)
        replay_btn.bind("<Leave>", replay_on_leave)
        
        def close_on_enter(e):
            close_btn.config(bg="#4b5563")
        def close_on_leave(e):
            close_btn.config(bg="#6b7280")
        close_btn.bind("<Enter>", close_on_enter)
        close_btn.bind("<Leave>", close_on_leave)
        
        # Fermer proprement si la fenêtre est fermée
        game_popup.protocol("WM_DELETE_WINDOW", close_game)
        
        # Démarrer le jeu après un court délai
        game_popup.after(800, show_psyduck_random)
